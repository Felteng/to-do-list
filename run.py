...
def complete_task():
    ...
    return to edit_task_list(), if user is sure, proceed with completion.
...
def remove_task(worksheet_name):
    ...
    proceed with removal, if user is not sure return to edit_task_list().
...
def back_to_edit(*origin_list):
    """Call if user presses enter with no input or with 'n' while editing"""
    print("Aborting editing and going back...")
    if origin_list == "Completed":
        display_completed_list()
